...
# F2FS format script
PRODUCT_COPY_FILES += \
    vendor/cm/prebuilt/common/bin/format.sh:system/bin/format.sh
...
ifeq ($(CM_BUILDTYPE), RELEASE)
    ...
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        CM_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(CM_BUILD)
    else
        CM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(CM_BUILD)
    endif
endif
